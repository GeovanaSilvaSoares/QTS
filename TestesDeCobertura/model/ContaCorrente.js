...
class ContaCorrente extends Conta {
    ...
    // Método para cobrar taxa fixa de R$20 no saldo da conta
    cobrarTaxa() {
        this.saldo = this.saldo - 20
        return { taxa: "Taxa de manutenção de R$20 debitada com sucesso" }
    }

    // Aplicação de taxa automaticamente
    saque(valor, confirmar = false) {
        const valorComTaxa = valor + 20 
        if (this.saldo >= valorComTaxa && confirmar) {
            this.saldo -= valorComTaxa
            return { saque: `Saque de ${valor} realizado com sucesso` }
        } else {
    ...
    transferir(origem, destino, valor) {
        const respSaque = origem.saque(valor, true) 
        if (respSaque.saque.includes("realizado com sucesso")) {
            destino.saldo += valor
