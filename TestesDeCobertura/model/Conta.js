...
class Conta{
    ...
    static autenticar(agencia, numero_conta, senha){
        let contas = Conta.contas
    
        var contaRetorno = null
        contas.forEach((conta) =>{
            if(conta.agencia == agencia && conta.senha == senha && conta.numero_conta== numero_conta){
    ...
    transferir(origem, destino, valor){
        ...
        // validar o destino
        let resp2 = Conta.autenticar(destino.agencia, destino.numero_conta, destino.senha)
        ...
        if(resp3.saque !== `Saque não realizado por falta de saldo`){
            destino.depositar(valor, true)
            return {transferencia: "Realizada com sucesso"}
        } else {
            return {transferencia: "Não realizada por falta de saldo"}
        }
    }
